...
def get_dataloaders(config):
    ...
    val_loader = data.DataLoader(
        MMDataset(data_path=config.data_path, mode='val',
                      modal=config.modal, fps=config.fps,
                      num_frames=config.num_segments, len_feature=config.len_feature,
...
class MMTrainer():
    ...
    def test(self):
        ...
        with torch.no_grad():
            targets, preds = [], []
            for _data, _data_audio, _label, _, _, _, _human_list in tqdm(self.test_loader):
                _data, _data_audio, _label, _human_list = _data.cuda(), _data_audio.cuda(), _label.cuda(), _human_list.cuda()
                x_cls, cas, action_flow, action_rgb = self.net(_data, _data_audio, _human_list, is_training=False)

                # score_supp = torch.sigmoid(x_cls)
    ...
    def evaluate_mutli_class(self, epoch=0, mode='val', data_loader=None):
        self.net = self.net.eval()
        with torch.no_grad():
            targets, preds = [], []
            for _data, _data_audio, _label, _, _, _, _human_list in tqdm(data_loader, desc="Evaluating '{}'".format(mode)):
                _data, _data_audio, _label, _human_list = _data.cuda(),_data_audio.cuda(), _label.cuda(), _human_list.cuda()
                x_cls, cas, action_flow, action_rgb = self.net(_data,_data_audio, _human_list, is_training=False)

                # If video-level detection
                # score_supp = torch.sigmoid(x_cls)
            ...
            if mAP_class > self.best_mAP_multi_class and mode == 'val':
                self.best_mAP_multi_class = mAP_class
                print("New best validation mAP: ", self.best_mAP_multi_class)
    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            for _data, _data_audio, _label, _, _, _, _human_list in tqdm(self.train_loader, desc='Training Epoch: {}'.format(epoch)):
                batch_size = _data.shape[0]
                _data, _data_audio, _label, _human_list = _data.cuda(), _data_audio.cuda(), _label.cuda(), _human_list.cuda()
                self.optimizer.zero_grad()
                # forward pass
                x_cls, cas_fg, action_flow, action_rgb = self.net(_data, _data_audio, _human_list, is_training=True)
            ...
            self.evaluate_mutli_class(epoch=epoch, mode='val', data_loader=self.val_loader)
            self.evaluate_mutli_class(epoch=epoch, mode='test', data_loader=self.test_loader)
            if epoch % 10 == 0:
                self.evaluate_mutli_class(epoch=epoch, mode='train', data_loader=self.train_loader)
