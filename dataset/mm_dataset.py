...
class MMDataset(Dataset):
    def __init__(self, data_path, mode, modal, fps, num_frames, len_feature, sampling, seed=-1, supervision='weak'):
        ...
        self.human_info = {}
        human_info_path = os.path.join("dataset", "human_information.json")
        if os.path.exists(human_info_path):
            with open(human_info_path, 'r') as f:
                self.human_info = json.load(f)
        else:
            print(">>> Human information not found!")
    ...
    def __getitem__(self, index):
        ...
        label, temp_anno, human_list = self.get_label(index, vid_num_frame, sample_idx)
        return data_video, data_audio, label, temp_anno, self.vid_list[index], vid_num_frame, human_list
    ...
    def get_label(self, index, vid_num_frame, sample_idx):
        ...
        ### HUMAN INFOR ###
        human_list = self.human_info[vid_name]
        # apply sample_idx
        human_list = [np.array(_)[sample_idx] for _ in human_list]
        human_list = torch.stack([torch.Tensor(_) for _ in human_list])
        ### ANNOTATION ###
        ...
        if self.supervision == 'weak':
            return label, torch.Tensor(0), human_list
        else:
            ...
            return label, torch.from_numpy(temp_anno), human_list
